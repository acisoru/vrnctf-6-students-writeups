...
engine = create_engine(
    f"postgresql+psycopg2://admin:admin@auth36rus-database:5432/{getenv('DB_NAME')}"
)
...
def create_tables():
    ...
    session = SyncSession()
    session.execute(text(f"INSERT INTO users (login, password) values ('admin', '{getenv('FLAG')}')"))
    session.commit()
